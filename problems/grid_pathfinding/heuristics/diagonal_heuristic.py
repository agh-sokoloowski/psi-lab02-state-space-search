...
class GridDiagonalHeuristic(Heuristic[GridCoord]):
    ...
    def __call__(self, state: GridCoord) -> float:
        ...
        delta_x = self.problem.diagonal_weight * abs(state.x - self.problem.goal.x)
        delta_y = self.problem.diagonal_weight * abs(state.y - self.problem.goal.y)
        return ((math.sqrt(2) - 1) * min(delta_x, delta_y)) + max(delta_x, delta_y)
